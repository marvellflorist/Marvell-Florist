...
- `admin/config.yml` sudah diarahkan ke local backend `http://localhost:8081/api/v1`.
...
Catatan Windows:
- `npm run cms:test` menjalankan server lokal dan proxy Decap sekaligus tanpa perlu Git Bash.
- Biarkan terminal tetap terbuka selama memakai CMS.
...
3. `admin/config.yml` masih mengarah ke `http://localhost:8081/api/v1`.
